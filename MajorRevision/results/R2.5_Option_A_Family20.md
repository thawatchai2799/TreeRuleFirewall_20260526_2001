# R2.5 Statistics — Option A: Full-Family Holm-Bonferroni (family = 20)

**Correction scope:** all 5 metrics × 4 policy sizes = 20 comparisons corrected together.
This is the most conservative, most defensible choice — hardest for a reviewer to
challenge, since it does not carve the family into sub-groups to make correction easier.
...
**17 of 20 comparisons remain significant after full family-wise correction.**
The 3 that do not (match latency and conversion time at n=200 and n=400) are exactly
the comparisons the paper already describes qualitatively as reaching "timing parity" --
the corrected statistics reinforce, rather than contradict, the paper's existing narrative.

**Effect size:** matched-pairs rank-biserial correlation r_rb = 1.000 for 17 of 20
comparisons (perfect separation -- protocol-first wins in every single trial), and
...
## Suggested manuscript text (Section VI-E / Table III replacement)
...
>
> Seventeen of the twenty comparisons remain significant after correction
...
> conversion time at n = 50 and n = 100. The three comparisons that do not
> survive correction -- match latency and conversion time at n = 200 and
> n = 400 -- are precisely the cases the original analysis already
...
## Suggested Response-to-Reviewers text (R2.5)

> **a)** The reviewer requested effect-size measures, correction for multiple
...
> Section VI-E and Table III have been rewritten to report effect sizes and
> Holm-adjusted p-values; 17/20 comparisons remain significant after
> correction, and the 3 that do not are exactly the cases we describe as
> reaching parity, now confirmed via TOST equivalence testing.

## Why this option is recommended

- Uses the full, undivided family -- a reviewer cannot argue we chose a
  favorable correction scope.
- Still yields 17/20 significant results with a very large effect size.
- The 3 "losses" align with, and strengthen, the paper's own existing
