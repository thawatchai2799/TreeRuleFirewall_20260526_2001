...
Figure files updated at the proofreading stage of the manuscript (journal style); no code, data,
or result file changed.

- Mathematical-object labels renumbered sequentially in the figures, following the manuscript:
  Definition 1.bis -> Definition 2 (Definitions 2-6 -> 3-7); Lemma 1.A -> Lemma 1, Lemma 1 -> 2,
  Lemma 1.B -> 3, Lemmas 2-4 -> 4-6; Corollary 1.B -> Corollary 1, Corollary 1 -> Corollary 2.
  Affected: Figure 7 (verdict line), Figure 8 (dependency chain), Figure 9 (level notes),
  Figure 13 (results box).
- Thousands separators follow the journal rule (comma only for five or more digits):
  Figure 1 and Figure 5 ("65,535"), Figure 10 and Figure 13 ("7000 trials").
- Cross-references removed from images: Figure 6 no longer cites Figure 12 / Table 6 (now
  "Sections 6.4 and 6.9"); Figure 14 legend no longer carries the reference number "[16]"
  (the citation is in the caption).
- `make_figure01.py` added, so Figure 1 is now generated by a script as well.
