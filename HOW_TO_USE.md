...
- **Ordering benchmark sample.** `ordering_benchmark_v12.py` generates its own policies with a simpler
  rule generator (single hosts, /24 subnets or ANY; single well-known ports; no anomaly injection):
  one policy per trial, so 10 trials = 10 independently generated policies per size. It does **not**
  read `policies_v12.jsonl.gz`. Each policy is converted under all 12 orderings and matched against the
  same 2,000 random packets. Paired statistics use the per-policy mean over orderings 1-6 versus 7-12.
- **FDD baseline.** `fdd_baseline_v12.py` runs this framework's own pipeline with the attribute ordering
  fixed to Ordering 1. It is an ordering-controlled ablation, not an independent implementation of
  Firewall Decision Diagrams; treat its absolute figures accordingly.

**Default ordering.** The framework default is Ordering 4. The follow-up study shows that Ordering 1
(protocol first, destination port last) gives far smaller trees (53.8% fewer nodes at n = 400);
select it with `--ordering 1` (see `ExtendedExperiments/AdaptiveOrdering/README.md`).
